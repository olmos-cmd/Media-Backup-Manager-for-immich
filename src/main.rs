...
impl ImmichApp {
    ...
    fn draw_header_logo(&self, ui: &mut egui::Ui) {
        if let Some(texture) = &self.header_logo_texture {
            // Das vom Benutzer gelieferte Logo wird proportional und scharf dargestellt.
            ui.add(egui::Image::new(texture).fit_to_exact_size(egui::vec2(360.0, 166.0)));
        }
    }
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            let header_decoded = image::load_from_memory(include_bytes!("../header_logo.png"))
                ...
                .into_rgba8();
            let header_size = [header_decoded.width() as usize, header_decoded.height() as usize];
            let header_color_image = egui::ColorImage::from_rgba_unmultiplied(
                header_size,
                header_decoded.as_raw(),
            );
