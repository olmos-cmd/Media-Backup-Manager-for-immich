...
struct ImmichApp {
    ...
    header_logo_texture: Option<egui::TextureHandle>,
...
impl ImmichApp {
            ...
            match event {
                ...
                DownloadEvent::Finished { downloaded, skipped, failed, conflicts, cancelled } => {
                    ...
                    self.status = if cancelled {
                        if self.english { format!("Cancelled. Downloaded: {} | Existing: {} | Errors: {}", downloaded, skipped, failed) }
                        else { format!("Abgebrochen. Heruntergeladen: {} | Vorhanden: {} | Fehler: {}", downloaded, skipped, failed) }
                    } else if self.english {
                        format!("Finished. Downloaded: {} | Existing: {} | Errors: {}", downloaded, skipped, failed)
                    } else {
                        format!("Fertig. Heruntergeladen: {} | Vorhanden: {} | Fehler: {}", downloaded, skipped, failed)
                    };
                    self.download_file = if cancelled {
                        if self.english { "Download cancelled.".to_string() } else { "Download abgebrochen.".to_string() }
                    } else if self.english { "Download completed.".to_string() } else { "Download abgeschlossen.".to_string() };
                    self.download_popup = false;
                    self.protocol_selected_albums = self.selected_album_count();
                    self.protocol_selected_years = self.selected_year_count() + self.selected_all_year_count();
    ...
    fn draw_header_logo(&self, ui: &mut egui::Ui) {
        if let Some(texture) = &self.header_logo_texture {
            // Original-PNG unverändert und proportional im tatsächlichen Seitenverhältnis anzeigen.
            ui.add(
                egui::Image::new(texture)
                    .fit_to_exact_size(egui::vec2(180.0, 88.9)),
            );
        }
    }
    ...
    fn ui_header(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::none()
            .fill(self.panel_bg())
            .inner_margin(egui::Margin::symmetric(18.0, 4.0))
            .show(ui, |ui| {
                // Kompakte Kopfzeile: Logo links, Beschreibung daneben, Bedienung rechts.
                ui.horizontal(|ui| {
                    ui.set_min_height(104.0);
                    self.draw_header_logo(ui);
                    ui.add_space(18.0);

                    ui.allocate_ui_with_layout(
                        egui::vec2(520.0, 102.4),
                        egui::Layout::top_down(egui::Align::LEFT),
                        |ui| {
                            ui.add_space(20.0);
                            let text_color = if self.dark_mode {
                                egui::Color32::from_rgb(218, 226, 233)
                            } else {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if ui.button(if self.dark_mode {
                            if en { "☀ Light" } else { "☀ Hell" }
                        } else if en { "🌙 Dark" } else { "🌙 Dunkel" }).clicked() {
                ...
                // Hauptbedienzeile: alle Verbindungs- und Downloadfunktionen bleiben fest zusammen.
                ui.horizontal_wrapped(|ui| {
                    ui.label(if en { "Immich server" } else { "Immich-Server" });
                    ui.add_sized([350.0, 30.0], egui::TextEdit::singleline(&mut self.server));

                    ui.add_space(6.0);
                    ui.label("API key");
                    if self.show_key {
                        ui.add_sized([285.0, 30.0], egui::TextEdit::singleline(&mut self.api_key));
                    } else {
                        ui.add_sized([285.0, 30.0], egui::TextEdit::singleline(&mut self.api_key).password(true));
                    }

                    if ui.add_sized([92.0, 30.0], egui::Button::new(
                        if self.show_key {
                            if en { "Hide" } else { "Verbergen" }
                        } else if en { "Show" } else { "Anzeigen" }
                    )).clicked() {
                    ...
                    if ui.add(test_button).clicked() {
                        match self.load_albums() {
                            ...
                            Err(e) => {
                                self.status = if en { format!("Error: {}", e) } else { format!("Fehler: {}", e) };
                            }
                        }
                    }
                ...
                // Sekundärzeile: alle Bedienelemente exakt auf einer gemeinsamen Höhe ausrichten.
                ui.scope(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        egui::ComboBox::from_id_salt("media_type_combo")
                            ...
                            .show_ui(ui, |ui| {
                                ui.selectable_value(
                                    &mut self.media_mode,
                                    MediaMode::All,
                                    if en { "Photos and videos" } else { "Fotos und Videos" },
                                );
                                ui.selectable_value(
                                    &mut self.media_mode,
                                    MediaMode::Photos,
                        ...
                        ui.checkbox(
                            ...
                            if en { "Shared albums" } else { "Geteilte Alben" },
                        );
                    });
                });
            });
        ...
        egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ...
                ui.add_sized(
                    ...
                    egui::TextEdit::singleline(&mut self.target_dir).hint_text(
                        if self.english {
    ...
    fn show_download_protocol_popup(&mut self, ctx: &egui::Context) {
        if !self.download_protocol_popup { return; }

        let mut open = self.download_protocol_popup;
        egui::Window::new(if self.english { "Download log" } else { "Protokoll" })
            .open(&mut open)
            .resizable(false)
            .movable(false)
            .collapsible(false)
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("download_protocol_grid")
                    ...
                    .show(ui, |ui| {
                        for (label, value) in [
                            (if self.english { "Selected albums" } else { "Ausgewählte Alben" }, self.protocol_selected_albums.to_string()),
                            (if self.english { "Selected years" } else { "Ausgewählte Jahre" }, self.protocol_selected_years.to_string()),
...
fn ui_albums_tab(&mut self, ui: &mut egui::Ui) {
    ...
    let card_height: f32 = 188.0_f32;
    let gap_x: f32 = 16.0_f32;
    let gap_y: f32 = 16.0_f32;
    let min_card_width: f32 = 330.0_f32;
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            // Die nutzbare Breite wird IM Scrollbereich bestimmt. Damit sind
            // Scrollbalken, Innenabstände und der tatsächliche Viewport bereits berücksichtigt.
            let viewport_width = ui.available_width().max(min_card_width);
            let scroll_clip = ui.clip_rect();

            // 1–6 Spalten, aber nur wenn die Mindestbreite pro Karte erhalten bleibt.
            let columns = (((viewport_width + gap_x) / (min_card_width + gap_x)).floor() as usize)
                .clamp(1, 6);

            let total_gap = gap_x * (columns.saturating_sub(1) as f32);
            let card_width = (viewport_width - total_gap) / columns as f32;

            for row in visible.chunks(columns) {
                // Eine komplette Rasterzeile reservieren. Dadurch beeinflusst keine Karte
                // die Position der nächsten Karte.
                let (row_rect, _) = ui.allocate_exact_size(
                    egui::vec2(viewport_width, card_height),
                    egui::Sense::hover(),
                );

                for (column, &index) in row.iter().enumerate() {
                    let x = row_rect.left() + column as f32 * (card_width + gap_x);
                    ...
                    // Die erlaubte Zeichenbreite richtet sich an der tatsächlichen Kartenbreite.
                    // Dadurch bleibt der Titel auch bei 4–6 Spalten kontrolliert innerhalb der Karte.
                    let chars_per_line = (((card_width - 178.0_f32) / 7.0_f32).floor() as usize)
                        .clamp(20, 44);
                    let display_name = Self::truncate_album_title(&full_name, chars_per_line, 5);
                    ...
                    let card_clip = card_rect.intersect(scroll_clip);
                    let card_painter = ui.painter().with_clip_rect(card_clip);
                    card_painter.rect_filled(card_rect, 10.0_f32, fill);
                    card_painter.rect_stroke(card_rect, 10.0_f32, stroke);

                    // Der gesamte Kartenbereich ist klickbar.
                    let mut response = ui.interact(
                        card_rect,
                        ui.id().with(("album_card", index)),
                        egui::Sense::click(),
                    );

                    let checkbox_rect = egui::Rect::from_min_size(
                        egui::pos2(card_rect.left() + 14.0_f32, card_rect.top() + 14.0_f32),
                        egui::vec2(24.0_f32, 24.0_f32),
                    );

                    let thumb_rect = egui::Rect::from_min_size(
                        egui::pos2(card_rect.left() + 48.0_f32, card_rect.top() + 14.0_f32),
                        egui::vec2(96.0_f32, 82.0_f32),
                    );

                    let title_rect = egui::Rect::from_min_max(
                        egui::pos2(thumb_rect.right() + 14.0_f32, card_rect.top() + 14.0_f32),
                        egui::pos2(card_rect.right() - 14.0_f32, card_rect.bottom() - 42.0_f32),
                    );
                    ...
                    checkbox_ui.set_clip_rect(checkbox_rect.intersect(scroll_clip));
                    let checkbox = checkbox_ui.add_sized(
                        [24.0_f32, 24.0_f32],
                        egui::Checkbox::new(&mut item.selected, ""),
                    );
                    let checkbox_clicked = checkbox.clicked();
                    checkbox.on_hover_text(if self.english {
                        "Select album"
                    } else {
                        "Album auswählen"
                    });

                    let thumb_clip = thumb_rect.intersect(scroll_clip);
                    if thumb_clip.is_positive() {
                        if let Some(texture) = &item.thumbnail {
                            ...
                            thumb_ui.set_clip_rect(thumb_clip);
                            thumb_ui.put(
                                thumb_rect,
                                egui::Image::new(texture).fit_to_exact_size(thumb_rect.size()),
                            );
                        } else {
                            let painter = ui.painter().with_clip_rect(thumb_clip);
                            painter.rect_filled(
                                thumb_rect,
                                8.0_f32,
                                if self.dark_mode {
                    ...
                    // Eigener Child-UI-Bereich sorgt dafür, dass Text niemals aus der Karte läuft.
                    let mut title_ui = ui.new_child(
                        egui::UiBuilder::new()
                            .max_rect(title_rect)
                            .layout(egui::Layout::top_down(egui::Align::LEFT)),
                    );
                    title_ui.set_clip_rect(title_rect.intersect(scroll_clip));

                    let title_response = title_ui.add_sized(
                    ...
                    let meta_y = card_rect.bottom() - 20.0_f32;
                    card_painter.text(
                        egui::pos2(card_rect.left() + 16.0_f32, meta_y),
                    ...
                    let badge_label = if item.album.shared {
                        if self.english { "shared" } else { "geteilt" }
                    } else if self.english {
                        "own"
                    } else {
                        "eigen"
                    ...
                    let badge_width = if item.album.shared {
                        if self.english { 62.0_f32 } else { 58.0_f32 }
                    } else {
                        48.0_f32
                    };
                    ...
                    let badge_rect = egui::Rect::from_center_size(
                        egui::pos2(
                            card_rect.right() - 14.0_f32 - badge_width / 2.0_f32,
                            meta_y,
                        ),
                        egui::vec2(badge_width, 22.0_f32),
                    );

                    card_painter.rect_filled(badge_rect, 9.0_f32, badge_fill);
                    card_painter.text(
                        badge_rect.center(),
                        egui::Align2::CENTER_CENTER,
                        badge_label,
                    ...
                    response = if item.selected {
                        response.on_hover_text(if self.english {
                        ...
                        response.on_hover_text(if self.english {
                            "Click to select the entire album"
                        } else {
                            "Klicken, um das gesamte Album auszuwählen"
                        })
                    };
                    let _ = response;
                }
...
fn ui_no_album_tab(&mut self, ui: &mut egui::Ui) {
        ...
        let columns: usize = if available >= 1320.0_f32 { 4 } else if available >= 960.0_f32 { 3 } else if available >= 640.0_f32 { 2 } else { 1 };
        let card_width = ((available - gap * (columns.saturating_sub(1) as f32)) / columns as f32).clamp(280.0_f32, 430.0_f32);
        let card_height: f32 = 92.0_f32;

        egui::ScrollArea::vertical()
            .id_salt("no_album_year_scroll")
            ...
            .min_width(620.0).min_height(420.0).open(&mut open).show(ctx, |ui| {
                ...
                egui::ScrollArea::vertical().id_salt("info_scroll").scroll_bar_visibility(egui::containers::scroll_area::ScrollBarVisibility::AlwaysVisible).auto_shrink([false,false]).show(ui, |ui| {
                    if en {
                        ...
                        ui.label("Media Backup Manager downloads the original files from Immich. After every larger backup, verify that the expected files are present and readable in the destination folder. The application does not replace an additional, regularly tested backup strategy.");
                        ui.add_space(12.0);
                        ui.heading("Freeware / rights");
                        ui.label("This application is freeware and may be used free of charge.");
                        ...
                        ui.label("Media Backup Manager lädt die Originaldateien aus Immich herunter. Nach jeder größeren Sicherung sollte geprüft werden, ob die erwarteten Dateien vollständig vorhanden und lesbar sind. Das Programm ersetzt kein zusätzliches, regelmäßig geprüftes Backup-Konzept.");
                        ui.add_space(12.0);
                        ui.heading("Freeware / Rechte");
                        ui.label("Dieses Programm ist Freeware und darf kostenlos genutzt werden.");
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::right("download_settings_panel")
            ...
            .frame(
                egui::Frame::none()
                    .fill(self.page_bg())
                    .inner_margin(egui::Margin { left: 10.0, right: 20.0, top: 10.0, bottom: 10.0 }),
            )
            .show(ctx, |ui| {
                self.ui_download_settings_panel(ui);
            });
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            let header_decoded = image::load_from_memory(include_bytes!("../header_logo.png"))
                .expect("Das eingebettete längliche Programmlogo konnte nicht geladen werden.")
                .into_rgba8();

            // Originaldatei bleibt unverändert. Für die GUI wird nur im Arbeitsspeicher
            // eine hochwertige Textur erzeugt, damit die Kanten beim Verkleinern sauber wirken.
            let header_filtered = image::imageops::resize(
                &header_decoded,
                720,
                356,
            ...
            let header_size = [
                header_filtered.width() as usize,
                header_filtered.height() as usize,
            ];
            let header_color_image = egui::ColorImage::from_rgba_unmultiplied(
                header_size,
                header_filtered.as_raw(),
            );
            app.header_logo_texture = Some(cc.egui_ctx.load_texture(
                "header_logo",
                header_color_image,
                egui::TextureOptions::LINEAR,
            ));
