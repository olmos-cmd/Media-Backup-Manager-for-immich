...
impl ImmichApp {
    ...
    fn draw_header_logo(&self, ui: &mut egui::Ui) {
        if let Some(texture) = &self.header_logo_texture {
            ...
            ui.add(egui::Image::new(texture).fit_to_exact_size(egui::vec2(180.0, 88.9)));
        }
    }
                            ...
                            Ok(()) => {
                                ...
                                if let Err(e) = save_settings(&self.server, &self.api_key) {
                                    self.status = if en {
                                        ...
                                        format!(
                                            "Verbindung erfolgreich, Speichern fehlgeschlagen: {}",
                                            e
                                        )
                                    };
                                }
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        let can_download = !self.download_running
                            && selected_count > 0
    ...
    fn ui_albums_tab(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let columns = (((viewport_width + gap_x) / (min_card_width + gap_x)).floor()
                    as usize)
                    .clamp(1, 6);
                ...
                for row in visible.chunks(columns) {
                    ...
                    for (column, &index) in row.iter().enumerate() {
                        ...
                        let chars_per_line =
                            (((card_width - 178.0_f32) / 7.0_f32).floor() as usize).clamp(20, 44);
                        let display_name =
                            Self::truncate_album_title(&full_name, chars_per_line, 5);
    ...
    fn ui_no_album_tab(&mut self, ui: &mut egui::Ui) {
        ...
        let columns: usize = if available >= 1320.0_f32 {
            4
        } else if available >= 960.0_f32 {
            3
        } else if available >= 640.0_f32 {
            2
        } else {
            1
        };
        let card_width = ((available - gap * (columns.saturating_sub(1) as f32)) / columns as f32)
            .clamp(280.0_f32, 430.0_f32);
                    ...
                    ui.horizontal(|ui| {
                        ...
                        for &index in row {
                            ...
                            let fill = if item.selected {
                                if dark_mode {
                                    egui::Color32::from_rgb(20, 38, 42)
                                } else {
                                    egui::Color32::from_rgb(232, 249, 250)
                                }
                            } else {
                                if dark_mode {
                                    egui::Color32::from_rgb(24, 30, 36)
                                } else {
                                    egui::Color32::WHITE
                                }
                            };
                            let stroke = if item.selected {
                                ...
                                egui::Stroke::new(
                                    1.0_f32,
                                    if dark_mode {
                                        egui::Color32::from_rgb(55, 67, 75)
                                    } else {
                                        egui::Color32::from_rgb(214, 219, 226)
                                    },
                                )
                                ...
                                ui.label(
                                    egui::RichText::new(if en {
                                        format!(
                                            "{} files · {}",
                                            item.count,
                                            Self::format_bytes_i64(item.total_size)
                                        )
                                    } else {
                                        format!(
                                            "{} Dateien · {}",
                                    ...
                                    .size(12.0_f32)
                                    .color(if dark_mode {
                                        egui::Color32::from_rgb(165, 177, 186)
                                    } else {
                                        egui::Color32::from_rgb(75, 85, 99)
                                    }),
                                );
                            });
                            if response.clicked() && !checkbox.clicked() {
                                item.selected = !item.selected;
                            }
                        }
                    });
    ...
    fn ui_footer(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            ui.label(if self.download_running {
                if self.english {
                    "Download running"
                } else {
                    "Download läuft"
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            let header_color_image =
                egui::ColorImage::from_rgba_unmultiplied(header_size, header_filtered.as_raw());
